...
class QSSegModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.visual_names = ['real_A', 'fake_B', 'real_B','realA_mask','seg_output']
        #self.visual_names = ['real_A', 'fake_B', 'real_B','realA_mask']
        ...
        self.netSeg=UNet(opt.input_nc,opt.num_classes).to(self.device)
        ...
        if self.isTrain:
            ...
            self.optimizer_G = torch.optim.Adam(list(self.netG.parameters())+list(self.netSeg.parameters()), lr=opt.lr, betas=(opt.beta1, opt.beta2))
    ...
    def set_input(self, input):
        ...
        self.realA_mask=input['A_mask' if AtoB else None].unsqueeze(1).to(self.device)
        self.image_paths = input['A_paths' if AtoB else 'B_paths']

        #print(f'realA unique values :{torch.unique(self.real_A)}')
        #print(f'realB unique values :{torch.unique(self.real_B)}')
        #print(f'rel A mask unique values :{torch.unique(self.realA_mask)}')
       
        #print(f'image shape :{self.real_A.shape}')
    ...
    def forward(self):
        ...
        self.seg_output=self.netSeg(self.fake_B)
    ...
    def backward_G(self):
        ...
        #tgt=one_hot(self.realA_mask, self.opt.num_classes, dim=1)
        #print(f'seg_output shape :{self.seg_output.shape}, realA_mask shape :{self.realA_mask.shape}')
        #print(f'realA_mask unique values : {torch.unique(self.realA_mask)}')
        self.realA_mask[self.realA_mask==-1]=0
        #print(f'realA_mask unique values : {torch.unique(self.realA_mask)}')
        self.loss_seg=self.criterionSeg(self.seg_output,self.realA_mask.squeeze(1).long()) 
