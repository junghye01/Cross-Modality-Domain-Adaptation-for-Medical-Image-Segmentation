...
def tensor2im2(input_image, imtype=np.uint8):
    ...
    if not isinstance(input_image, np.ndarray):
        if isinstance(input_image, torch.Tensor):  # get the data from a tensor
       ...
       #     image_numpy=image_numpy[0]
        ...
        image_numpy = np.transpose(image_numpy, (1, 2, 0))  # Transpose to (H, W, C) shape
...
def save_image(image_numpy, image_path, aspect_ratio=1.0):
    ...
    #image_pil = Image.fromarray((image_numpy * 255).astype(np.uint8), mode='L')
...
def save_image_as_dicom(image_numpy, image_path, aspect_ratio=1.0):
    """Save a numpy image to the disk as a DICOM file
    ...
    if aspect_ratio is not None:
        ...
        if aspect_ratio > 1.0:
            spacing[0] *= aspect_ratio
        elif aspect_ratio < 1.0:
            spacing[1] /= aspect_ratio
