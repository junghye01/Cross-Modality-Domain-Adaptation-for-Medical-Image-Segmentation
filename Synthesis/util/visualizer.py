...
def save_images(webpage, visuals, image_path, aspect_ratio=1.0, width=256):
    ...
    for label, im_data in visuals.items():
        im = util.tensor2im2(im_data)
        image_name = '%s/%s.dcm' % (label, name)
        os.makedirs(os.path.join(image_dir, label), exist_ok=True)
        save_path = os.path.join(image_dir, image_name)
        util.save_image_as_dicom(im,save_path,aspect_ratio=aspect_ratio)
        ...
        if self.display_id > 0:  # show images in the browser using visdom
            ...
            if ncols > 0:        # show all the images in one visdom panel
                ...
                for label, image in visuals.items():
                    ...
                    image_numpy = util.tensor2im2(image)
                    #print(f'{label} image numpy shape :{image_numpy.shape}')
                    #print(f'{label} unique values : {np.unique(image_numpy)}')
                    label_html_row += '<td>%s</td>' % label
                    images.append(image_numpy.transpose([2, 0, 1])) # h,w,c c,h,w
                    idx += 1
                    if idx % ncols == 0:
