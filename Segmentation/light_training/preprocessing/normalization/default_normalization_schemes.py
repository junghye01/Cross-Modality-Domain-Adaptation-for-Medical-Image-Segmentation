...
class CTNormalization(ImageNormalization):
    ...
    def run(self, image: np.ndarray, seg: np.ndarray = None) -> np.ndarray:
        ...
        lower_bound = self.intensityproperties['percentile_00_5'] # 하위 0.5 백분위수값
        upper_bound = self.intensityproperties['percentile_99_5'] # 상위 99.5 백분위수값
        image = np.clip(image, lower_bound, upper_bound) # 아웃라이어 픽셀 값들이 평균과 표준편차 계산에 영향을 미치지 않기 위하여
