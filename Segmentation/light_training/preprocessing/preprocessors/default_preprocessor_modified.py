...
class DefaultPreprocessor(object):
    ...
    # 입력 데이터의 복사본을 생성하여 원본을 보호합니다.
        ...
        # 크롭 전의 이미지 크기 저장
        shape_before_cropping = data.shape[1:] # (depth, width, height)
        properties['shape_before_cropping'] = shape_before_cropping
        
        # 이미지를 크롭하고 크롭에 사용된 bounding box 저장
        
        print(f'nontype? : {np.sum(seg)}')
        data, seg, bbox = crop_to_nonzero(data, seg) # dim 4
            
        properties['bbox_used_for_cropping'] = bbox
        

        # 크롭 후 리샘플링 전의 이미지 크기 저장
        shape_before_resample = data.shape[1:]
        properties['shape_after_cropping_before_resample'] = shape_before_resample

        if self.out_spacing is None:
            print(f'No resampling')
            properties['shape_after_resample']=shape_before_resample
            ...
            # 세그멘테이션 데이터가 없는 경우, 세그멘테이션을 전처리할 때 문제가 발생하지 않도록 빈 배열 생성
            if seg is None:
            ...
            # 이미지 데이터 정규화
            data = self._normalize(data, seg_norm, self.foreground_intensity_properties_per_channel)
            assert len(data.shape) == 4  # 데이터는 4D (채널, 높이, 너비, 깊이)이어야 합니다.
            ...
            data = resample_data_or_seg_to_shape(data, new_shape, 
                                                ...
                                                order_z=0)  # order_z는 보통 0 (nearest neighbor)로 설정
    
            properties['shape_after_resample'] = new_shape

            if seg is not None:
                print(f'seg 있음!!!!!!!!!!')
                assert len(seg.shape) == 4  # 세그멘테이션 데이터는 4D이어야 합니다.
        ...
        #print(f'old shape: {old_shape}, old_spacing: {original_spacing}, '
        #    f'new_spacing: {self.out_spacing}')
        print(f'old shape: {old_shape}, old_spacing : {original_spacing}, shape before cropping : {shape_before_cropping}, shape after cropping:{shape_before_resample},'
              f'bbox used for cropping : {bbox}')
    ...
    def _normalize(self, data: np.ndarray, seg: np.ndarray, foreground_intensity_properties_per_channel: dict) -> np.ndarray:
        ...
        # need to modify
    def read_data(self, file_path,seg_dir):
        ## CT 데이터셋 전용
        print(f'혹시 아니겄지..')
        # 이미지 데이터 읽기
        data = sitk.ReadImage(file_path)
        
        # case_name 추출 (예: 10_SJB_Chest__1.5_B50f_AQ_0000.nii.gz -> 10_SJB_Chest__1.5_B50f_AQ)
        case_name = os.path.basename(file_path)[:-12]
        
        # 이미지를 NumPy 배열로 변환 (float32 형식)
        data_arr = sitk.GetArrayFromImage(data).astype(np.float32)
        data_arr = data_arr[None]  # 새로운 축을 추가하여 (1, H, W, D) 형식으로 만듭니다.
        ...
        if seg_dir is not None:
            seg_file_path = os.path.join(self.base_dir, seg_dir, f'{case_name}_MASK.nii.gz')
            if seg_file_path:
                seg = sitk.ReadImage(seg_file_path)
                seg_arr = sitk.GetArrayFromImage(seg).astype(np.float32)
                seg_arr = seg_arr[None]  # 세그멘테이션도 새로운 축 추가

            # 전경(intensities)의 강도 정보를 수집
                
                intensities_per_channel, intensity_statistics_per_channel = self.collect_foreground_intensities(seg_arr, data_arr)
            else:
                print(f'{seg_file_path} does not exist')
        # 이미지와 세그멘테이션의 속성(properties) 정의
        properties = {
            "spacing": data.GetSpacing(), 
            "raw_size": data_arr.shape[1:],  # (H, W, D) 크기
            "name": case_name,
    ...
    def run_case(self, file_path):
        ...
        # 데이터와 세그멘테이션 로드
        data, seg, properties = self.read_data(file_path)

        # 전처리 단계 실행 (예: numpy 배열 형태로 전처리)
        data, seg = self.run_case_npy(data, seg, properties)

        return data, seg, properties 

    def run_case_save(self, file_path):
    # 파일명에서 case_name을 추출
        case_name = os.path.basename(file_path)[:-12]
        ...
        # 전처리 수행
        data, seg, properties = self.run_case(file_path)

        # 출력 파일명 생성
        output_base_name = os.path.join(self.output_dir, case_name)

        # 데이터와 세그멘테이션 결과를 .npz 형식으로 저장
        np.savez_compressed(output_base_name + '.npz', data=data, seg=seg)

        # 전처리 속성(properties)을 .pkl 파일로 저장
        write_pickle(properties, output_base_name + '.pkl')

        print(f"data is saved at: {output_base_name}.npz")
