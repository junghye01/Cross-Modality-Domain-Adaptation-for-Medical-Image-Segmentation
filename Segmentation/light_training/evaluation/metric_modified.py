...
class ConfusionMatrix:
    ...
    def set_test(self, test):
        self.test = np.asarray(test) if test is not None else None
        self.reset()
    ...
    def get_existence(self):
        if any(x is None for x in [self.test_empty, self.test_full, self.reference_empty, self.reference_full]):
            self.compute()
    ...
    def plot_confusion_matrix(self, save_path='confusion_matrix.png'):
        ...
        """
        # Ensure the confusion matrix is computed
        if self.tp is None or self.fp is None or self.tn is None or self.fn is None:
            self.compute()
        ...
        sns.heatmap(cm, annot=True, fmt='d', cmap='Blues', cbar=False,
                    xticklabels=['Positive (True)', 'Negative (True)'],
                    yticklabels=['Positive (Pred)', 'Negative (Pred)'])
