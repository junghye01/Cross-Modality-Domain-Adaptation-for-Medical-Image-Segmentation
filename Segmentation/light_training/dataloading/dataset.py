...
class MedicalDataset(Dataset):
    def __init__(self, datalist, test=False) -> None:
        ...
        for f in folder:
            unpack_dataset(f, 
                        unpack_segmentation=True, # if no seg
    ...
    def read_data(self, data_path):
        ...
        seg_data = None 
        if not self.test and os.path.exists(seg_path): # 이 부분 수정
            seg_data = np.load(seg_path, "r+")
        else:
            print(f'no seg in {data_path}')
            seg_data=np.zeros_like(image_data)
    ...
    def __getitem__(self, i):
        ...
        if properties is not None:
            properties = {k: v if v is not None else [] for k, v in properties.items()}  # None 대신 빈 리스트로 대체
...
def get_train_val_test_loader_from_train(data_dir, train_rate=0.8, val_rate=0.1, test_rate=0.1, seed=42):
    ## train all labeled data 
    ## fold denote the validation data in training data
    all_paths = glob.glob(f"{data_dir}/*.npz")
    # fold_data = get_kfold_data(all_paths, 5)[fold]
...
def get_train_val_loader_from_train(data_dir, train_rate=0.9, val_rate=0.1, seed=42):
    ## train all labeled data 
    ## fold denote the validation data in training data
     # 모든 .npz 파일의 경로를 가져옵니다.
    all_paths = glob.glob(f"{data_dir}/*.npz")

    
    # train과 validation 데이터 수를 계산합니다.
    total_number = len(all_paths)
    print(f'# of total data :{total_number}')
    ...
    # MedicalDataset 클래스를 사용하여 데이터셋을 만듭니다.
    train_ds = MedicalDataset(train_datalist)
    val_ds = MedicalDataset(val_datalist)
